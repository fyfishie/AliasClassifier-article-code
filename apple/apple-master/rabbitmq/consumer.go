...
func (c *Consumer) connect() (err error) {
	logrus.Info("consumer tring to connect...")
...
func (c *Consumer) RunAsPushMod(prefetchCount int) (taskin chan amqp.Delivery, err error) {
	...
	if !c.connected {
		logrus.Panic("cant not connect to mq, err =" + err.Error())
		return nil, err
	}
	// c.rabbit.channel.Qos(prefetchCount, -1, false)
	deliveries, err := c.rabbit.channel.Consume(c.queueInfo.Name, c.bindKey, false, false, false, true, amqp.Table{})
	if err != nil {
...
func (c *Consumer) clean() {
	...
	c.rabbit.confirms = make(chan amqp.Confirmation)
	c.rabbit.errSigChan = make(chan *amqp.Error)
}
